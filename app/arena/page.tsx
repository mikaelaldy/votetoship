...
function ArenaContent() {
  ...
  useEffect(() => {
    const savedIdeas = loadJSON<Idea[]>(STORAGE_KEYS.ideas, []);
    const savedVotes = loadJSON<Record<string, Votes>>(
      STORAGE_KEYS.votes,
      {}
    );

    if (shouldReset || savedIdeas.length === 0) {
      setIdeas([]);
      setVotes({});
      saveJSON(STORAGE_KEYS.votes, {});
    } else {
      setIdeas(savedIdeas);
      setVotes(savedVotes);
    }
  ...
  const generateIdeas = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setVotes({});
        saveJSON(STORAGE_KEYS.votes, {});
        saveJSON(STORAGE_KEYS.ideas, data.ideas);
      }
    } catch {
      // silent
    } finally {
  ...
  useEffect(() => {
    ...
    const needsGeneration =
      shouldReset || loadJSON<Idea[]>(STORAGE_KEYS.ideas, []).length === 0;
    if (needsGeneration) {
  ...
  }, [allVoted, ideas.length, router, votes]);
