...
function ArenaContent() {
  ...
  useEffect(() => {
    const unsubIdeas = subscribeToIdeas((newIdeas) => {
      setIdeas(newIdeas);
    });

    const unsubVotes = subscribeToVotes((newVotes) => {
      const mapped: Record<string, Votes> = {};
      for (const [id, v] of Object.entries(newVotes)) {
        const val = v as { up?: number; down?: number };
        mapped[id] = { up: val?.up ?? 0, down: val?.down ?? 0 };
      }
      setVotes(mapped);
    });
  ...
  useEffect(() => {
    if (!hydrated) return;
    if (shouldReset || ideas.length === 0) {
      generateIdeas();
    }
  }, [hydrated, shouldReset]);
  ...
  const generateIdeas = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await setCurrentIdeas(data.ideas);
      }
    } catch {
      // silent
    } finally {
  ...
  const handleVote = useCallback(
    async (ideaId: string, direction: "up" | "down") => {
      await castVote(ideaId, direction);
    },
    []
  );
  ...
  const votedCount = useMemo(() => {
    return ideas.filter((idea) => {
      const v = votes[idea.id];
      return v && (v.up > 0 || v.down > 0);
    }).length;
  }, [ideas, votes]);

  const allVoted = ideas.length > 0 && votedCount === ideas.length;

  useEffect(() => {
    if (!allVoted || ideas.length === 0 || countdown !== null) return;
    setCountdown(3);
  ...
  }, [allVoted, ideas.length, router, countdown]);
